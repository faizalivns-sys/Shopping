...
function loadCategoryProducts(category) {
    ...
    // Handle special case for "home-kitchen" which comes as "home-kitchen" from URL
    if (normalizedCategory === "home-&-kitchen") {
        normalizedCategory = "home-kitchen";
    }
    ...
    let products = [];
    if (categoryProducts[normalizedCategory]) {
        products = categoryProducts[normalizedCategory];
    } else {
        // Fallback to all products if category not found
        products = categoryProducts['all'];
    }
...
// Render products to the category page
function renderCategoryProducts(products) {
    categoryProductsGrid.innerHTML = '';
    
    if (products.length === 0) {
        categoryProductsGrid.innerHTML = '<p>No products found in this category.</p>';
        return;
    }
...
function addToCart(productId) {
    ...
    for (const category in categoryProducts) {
        const found = categoryProducts[category].find(p => p.id === productId);
        if (found) {
            product = found;
            break;
        }
...
if (categorySearchInput) {
    ...
    categorySearchInput.addEventListener('blur', function() {
...
if (categorySearchButton) {
    categorySearchButton.addEventListener('click', function() {
        const searchTerm = categorySearchInput ? categorySearchInput.value.trim() : '';
        if (searchTerm !== '') {
            // Redirect to search results page
            window.location.href = `search.html?q=${encodeURIComponent(searchTerm)}`;
            clearSuggestions();
...
function showSuggestions(term) {
    ...
    if (!suggestionsContainer) {
        ...
        suggestionsContainer.style.cssText = `
            ...
            overflow-y: auto;
        `;
        categorySearchInput.parentNode.style.position = 'relative';
        categorySearchInput.parentNode.appendChild(suggestionsContainer);
    }
    ...
    matches.forEach((match, index) => {
        ...
        suggestionItem.addEventListener('click', function() {
            if (categorySearchInput) {
                categorySearchInput.value = match;
                // Redirect to search results page
