...
// DOM Elements
const electronicsProductsGrid = document.getElementById('electronics-products-grid');
...
function loadElectronicsProducts() {
    ...
    productCount.textContent = `Showing ${electronicsProducts.length} products`;
    
    // Render products
    renderElectronicsProducts(electronicsProducts);
}

// Render electronics products to the page
function renderElectronicsProducts(products) {
    electronicsProductsGrid.innerHTML = '';
    
    if (products.length === 0) {
        electronicsProductsGrid.innerHTML = '<p>No products found in this category.</p>';
        return;
    }
    ...
    products.forEach(product => {
        ...
        `;
        electronicsProductsGrid.appendChild(productCard);
    });
}
...
// Call updateAccountLink when the page loads
updateAccountLink();
...
const electronicsSearchInput = document.getElementById('search-input');
const electronicsSearchButton = document.getElementById('search-button');

if (electronicsSearchInput) {
    electronicsSearchInput.addEventListener('keyup', function(e) {
        if (e.key === 'Enter') {
    ...
    electronicsSearchInput.addEventListener('blur', function() {
...
if (electronicsSearchButton) {
    electronicsSearchButton.addEventListener('click', function() {
        const searchTerm = electronicsSearchInput ? electronicsSearchInput.value.trim() : '';
        if (searchTerm !== '') {
            // Redirect to search results page
            window.location.href = `search.html?q=${encodeURIComponent(searchTerm)}`;
            clearSuggestions();
...
function showSuggestions(term) {
    ...
    if (!suggestionsContainer) {
        ...
        suggestionsContainer.style.cssText = `
            ...
            overflow-y: auto;
        `;
        electronicsSearchInput.parentNode.style.position = 'relative';
        electronicsSearchInput.parentNode.appendChild(suggestionsContainer);
    }
    ...
    matches.forEach((match, index) => {
        ...
        suggestionItem.addEventListener('click', function() {
            if (electronicsSearchInput) {
                electronicsSearchInput.value = match;
                // Redirect to search results page
                window.location.href = `search.html?q=${encodeURIComponent(match)}`;
                clearSuggestions();
                electronicsSearchInput.focus();
            }
        });
