...
const homepageSearchInput = document.querySelector('.search-bar input');
if (homepageSearchInput) {
    homepageSearchInput.addEventListener('keyup', function(e) {
        if (e.key === 'Enter') {
...
function renderFilteredProducts(filteredProducts) {
    ...
    if (filteredProducts.length === 0) {
        productsGrid.innerHTML = '<p>No products found. Please try another search term.</p>';
        return;
    }
...
function showSuggestions(term) {
    ...
    if (!suggestionsContainer) {
        ...
        globalSearchInput.parentNode.appendChild(suggestionsContainer);
    }
